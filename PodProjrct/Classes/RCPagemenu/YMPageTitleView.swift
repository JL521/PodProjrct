...
class YMPageTitleView: UIView {
    ...
    fileprivate lazy var scrollLine : UIView = {
        ...
        scrollLine.backgroundColor = UIColor(redc: kLineColor.0, green: kLineColor.1, blue: kLineColor.2)
        return scrollLine
    }()
...
//        let labelW : CGFloat = 120
        ...
        for (index, title) in titles.enumerated() {
            ...
            label.textColor = UIColor(redc: kNormalColor.0, green: kNormalColor.1, blue: kNormalColor.2)
    ...
    fileprivate func setupBottomLineAndScrollLine() {
        ...
        bottomLine.backgroundColor = UIColor(redc: kBottomLineColor.0, green: kBottomLineColor.1, blue: kBottomLineColor.2)
        ...
        firstLabel.textColor = UIColor(redc: kSelectColor.0, green: kSelectColor.1, blue: kSelectColor.2)
...
extension YMPageTitleView {
    @objc fileprivate func titleLabelClick(_ tapGes : UITapGestureRecognizer) {
        ...
        // 3.切换文字的颜色
        currentLabel.textColor = UIColor(redc: kSelectColor.0, green: kSelectColor.1, blue: kSelectColor.2)
        oldLabel.textColor = UIColor(redc: kNormalColor.0, green: kNormalColor.1, blue: kNormalColor.2)
        // 4.保存最新Label的下标值
...
extension YMPageTitleView {
    func setTitleWithProgress(_ progress : CGFloat, sourceIndex : Int, targetIndex : Int) {
        ...
        // 3.2.变化sourceLabel
        sourceLabel.textColor = UIColor(redc: kSelectColor.0 - colorDelta.0 * progress, green: kSelectColor.1 - colorDelta.1 * progress, blue: kSelectColor.2 - colorDelta.2 * progress)
        // 3.2.变化targetLabel
        targetLabel.textColor = UIColor(redc: kNormalColor.0 + colorDelta.0 * progress, green: kNormalColor.1 + colorDelta.1 * progress, blue: kNormalColor.2 + colorDelta.2 * progress)
        // sourceLabel缩放
...
extension YMPageTitleView {
    func refreshContentSize() {
        ...
        let leftx = CGFloat(integerLiteral: currentIndex) * cellWidth + cellWidth*0.5
        let rightx = contentsize.width - leftx
        var tarx = 0.0
        if leftx < width / 2.0 {
            tarx = 0.0
        } else if rightx < width / 2.0 {
            tarx = Double(contentsize.width - width)
        } else {
            tarx = Double(leftx - width / 2.0)
        }
        scrollView.setContentOffset(CGPoint(x: tarx, y: 0), animated: true)
    }
